...
```mermaid
graph TD
    A[Arabic PDF URL / Local PDF] -->|Stage 1: src/main.py| B[ocr_response.json]
    B -->|Mistral OCR markdown concat| C[output.md]
    C -->|Stage 2: src/split_output.py| D[heading_cache.json]
    C -->|Section-based Splitting| E[chunks/ 01_slug.md, 02_slug.md...]
    E -->|Stage 3: src/translate_chunks.py| F[translated_chunks/]
...
* **[src/main.py](file:///Users/firozahmed/Desktop/transcribe/src/main.py)**: Initiates Mistral OCR, parses response pages, and generates `output.md`.
